...
public class LookupSwitchGenerator {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        String outputClassName = "LookupSwitchInstructionSpecialCase";
        ...
        builder.append(String.format("public class LookupSwitchInstructionSpecialCase implements InstructionAnalyzer {%n"));
        builder.append(String.format("    @Instruction(value = 0xab, name = \"lookupswitch\")%n"));
        builder.append(String.format("    public void test(int number) {%n"));
        builder.append(String.format("        int answer = switch (number) {%n"));

        builder.append(String.format("            case %s -> %s;%n", Integer.MIN_VALUE, 42));
        int ratio = 10;
        int delta = -10100;
        for (int i = 0; i <= 10; i++) {
